...
```jsx
ReactDOM.render(<Title>Hello world</Title>, root)
```
...
2. Create your own _EssayBody_ component with a `<span>` and a `<p>` element inside it. The `<span>` should hold the
 author's name and the `<p>` should hold the actual text of the essay. Create an _Essay_ component too, which consists 
 of a _Title_ component and your newly created _EssayBody_. Use _props_ to pass data between the components.
